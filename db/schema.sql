...
create table public.shops (
  ...
  -- Per-shop order numbering: SAV-1042 reads better on an invoice and in a
  -- phone call than a UUID, and numbers must not interleave across brands.
...
-- ---------------------------------------------------------------------------
-- Per-shop order numbers, race-safe.
-- ---------------------------------------------------------------------------
...
begin
  ...
  where id = p_shop_id
  returning order_seq, order_prefix into v_seq, v_prefix;
...
-- deliberately NO anon policies. Service role only.
